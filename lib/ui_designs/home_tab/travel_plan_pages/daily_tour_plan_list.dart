...
class DailyTourPlanList extends StatefulWidget {
  ...
  const DailyTourPlanList({Key? key,required this.tourPlanId, required this.tourPlanDate, required this.status}) : super(key: key);
...
class DailyTourPlanListState extends State<DailyTourPlanList> {
  ...
  List<DateTime> generateDateList(int month) {
    ...
    for (var day = firstDay; day.isBefore(lastDay) ||
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: CustomAppBar(
          title: "${getMonthName(widget.tourPlanDate.month)} ${widget.tourPlanDate.year}",implyStatus: true,
          actions: [
            Padding(
        ...
        body: Column(
          children: [
            ...
            !isLoading ? dailyTourData.isNotEmpty ?
            Expanded(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.center,
                                children: [
                                  Text(dayName.substring(0,3).toUpperCase(),style: TextStyle(
                                    fontSize: 11,fontWeight: FontWeight.w600,
                                    color: dayName.substring(0,3).toUpperCase() == "SUN" ? Constants.primaryColor : Colors.black
                                  ),),
                                  Text("${currentDate.day}",style: const TextStyle(
                                      fontSize: 19
                                  ),),
                                ],
                              ),
                              ...
                              Column(
                                children:[
                                  ...
                                  //add
                                  if(!(dailyTourData[index].holidayData !=null &&dailyTourData[index].holidayData!.isNotEmpty) &&
                                      !(dailyTourData[index].leaveData !=null) && !(dailyTourData[index].tourPlan !=null && dailyTourData[index].tourPlan!.isNotEmpty) && widget.status !="Approved")
                                    Container(
                                          ...
                                          children: [
                                              ...
                                              onTap: () async {
                                                if(widget.status != "Approved"){
                                                  var response = await showDialog(context: context,
                                                      ...
                                                      builder: (_){
                                                    return AddTourPlanDialog(canEdit: false, date: dailyTourData[index].date!, tourPlanId: widget.tourPlanId, status: widget.status,);
                                                  });
                                                  if(response!=null){
                                                    isEdit=true;
                                                    dailyTourData.clear();
                                                    getDailyPlanData(response[0],response[1]);
                                                  }
                                                }
                                              },
                                              child: Container(
                                                ...
                                                child: const Text("Tap to add tour plan",
                                                style: TextStyle(
                                                  color: Colors.white,
                                            ...
                                            GestureDetector(
                                              onTap: () async {
                                                if(widget.status != "Approved"){
                                                  var result = await showDialog(context: context, builder: (_){
                                                    return AddLeaveDialog(date: dailyTourData[index].date!,tourPlanId: widget.tourPlanId);
                                                  });

                                                  if(result!=null){
                                                    isEdit=true;
                                                    dailyTourData.clear();
                                                    getDailyPlanData(result[0], result[1]);
                                                  }
                                                }
                                              ...
                                              child: Container(
                                                ...
                                                child: const Text("Mark as leave",style: TextStyle(
                                                  fontSize: 12,
                                                  color: Colors.white
                                                ),),
                                              ),
                                  ...
                                  if(widget.status =="Approved" &&  dailyTourData[index].tourPlan!.isEmpty )
                                  Visibility(
                                    visible: dailyTourData[index].leaveData ==null && dailyTourData[index].holidayData!.isEmpty,
                                    child: Container(
                                      width: 88.w,
                                      alignment: Alignment.center,
                                      margin: const EdgeInsets.symmetric(horizontal: 3,vertical: 4),
                                      ...
                                      child: const Text("No Data Available ",style: TextStyle(
                                          color: Colors.black
                                      ),
                                      ),
                                    ),
                                  ...
                                  if((dailyTourData[index].holidayData !=null && dailyTourData[index].holidayData!.isNotEmpty))
                                    Container(
                                      ...
                                      child: Text(dailyTourData[index].holidayData?.first.holidayName ?? "",style: const TextStyle(
                                        color: Colors.white
                                      ),
                                      ),
                                    ),
                                  ...
                                  //leave
                                  if(dailyTourData[index].leaveData!=null )
                                    Container(
                                      width: 88.w,
                                      alignment: Alignment.center,
                                      margin: const EdgeInsets.symmetric(horizontal: 3,vertical: 4),
                                      ...
                                      child: Text(dailyTourData[index].leaveData?.leaveType ?? "",style: const TextStyle(
                                          color: Colors.white
                                      ),
                                      ),
                                    ),

                                  ...List.generate(dailyTourData[index].tourPlan!.length, (pos) {
                                    return GestureDetector(
                                      onTap:() async {
                                          var result = await Get.to(TourPlanDetailsPage(tourPlanList: dailyTourData[index].tourPlan!,
                                              date: dailyTourData[index].date!, tourPlanId: widget.tourPlanId, status: widget.status),
                                      ...
                                      child: Container(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                Text(dailyTourData[index].tourPlan?[pos].activityType?.activityTypeName ?? "",
                                                ...
                                                const Spacer(),
                                                 Icon(Icons.circle,color:dailyTourData[index].tourPlan?[pos].status == "Created"
                                                    ? Colors.white
                                                    : dailyTourData[index].tourPlan?[pos].status == "Started"
                                                    ? Colors.green
                                                    : dailyTourData[index].tourPlan?[pos].status == "Paused"
                                                    ? Colors.orange
                                                     : dailyTourData[index].tourPlan?[pos].status == "Resumed"
                                                   ? Colors.blue :Colors.red,size: 8,)
                                              ],
                                            ),
                                            Text("Area : ${dailyTourData[index].tourPlan?[pos].area?.townName ?? ""}",
                                              style: const TextStyle(
                                                  fontSize: 12,
                                                  color: Colors.black
                                              ),
