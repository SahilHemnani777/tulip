...
class BrandSearchDelegate extends SearchDelegate {
  ...
  final _pagingController = PagingController<int, BrandItem>(
  ...
  @override
  String get searchFieldLabel => "Search Brand";


  void apiCall(int pageKey) {
    // Check if the query is not empty before making the API call
    if (query.isNotEmpty) {
      CustomerRepo.getBrandList(pageKey.toString(), query)
          .then((result) {
        if (result.status) {
          if (result.data != null) {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    return PagedListView<int, BrandItem>(
      pagingController: _pagingController,
      builderDelegate: PagedChildBuilderDelegate<BrandItem>(
        itemBuilder: (context, item, index) {
          return GestureDetector(
              ...
              child: Container(
                  ...
                  child: Text(item.productBrandName ?? "")));
        },
