...
class _LoginPageState extends State<LoginPage> {
        ...
        child: Container(
          ...
          decoration: const BoxDecoration(
            image: DecorationImage(
              image: AssetImage("assets/login_images/login_bg_image.png"),fit: BoxFit.fill
            )
          ),
          child: Column(
            ...
            children: [
              ...
              CustomButton(
                 ...
                 title: showPasswordField ? "Login" : "Next",onTabButtonCallback: validateLogin),
  ...
  Future<void> forgotPasswordClick() async {
    if(_mobileNumber.text.isEmpty){
      ...
      if(await InternetUtil.isInternetConnected()){
        ...
        try{
          var response = await LoginMaster.forgotPassword(_mobileNumber.text,_emailId.text);
          if(response.status){
            Get.back();
            SessionManager.saveUserId(response.data!.id);
            context.showSnackBar("OTP send successfully",null);
            Get.to(()=>OTPPage(emailId: _emailId.text, mobileNumber: _mobileNumber.text,fromForgotPassword: true, userID: response.data!.id));
          }
          else{
            Get.back();
  ...
  Future<void> sendOTP(String userId) async {
      try{
        ...
        if(result.status){
          ...
          Get.back();
          Get.to(OTPPage(emailId: _emailId.text,mobileNumber: _mobileNumber.text, fromForgotPassword: false, userID: result.data,),
    ...
    if (_mobileNumber.text.isNotEmpty && _mobileNumber.text.length == 10) {
      if(_emailId.text.isNotEmpty && _emailId.text.validateEmailId()){
        if(!showPasswordField) {
          ...
          context.showSnackBar("Please enter valid password",Constants.primaryColor);
        }else {
          checkUserLogin();
        }
      }
        ...
        context.showSnackBar("Please enter valid email",Constants.primaryColor);
      }
    }
    else {
      setState(() {
      ...
      context.showSnackBar("Please Enter Valid Mobile Number",Constants.primaryColor);
    }
  }
  ...
  Future<void> checkUserExists() async {
    ...
    if(await InternetUtil.isInternetConnected()) {
      ProgressDialog.showProgressDialog(context);
      // try {
        final value = await LoginMaster.checkUserExists(_mobileNumber.text,_emailId.text);
